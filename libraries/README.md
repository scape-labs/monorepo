...
framework (`kit/`, `dsl/`, `compiler/`). This `libraries/` tree owns domain types
...
## When to add to `libraries/` vs `services/<svc>/`

- **Add to `libraries/`** when ≥2 services need the same type/function and the dependency would otherwise be a copy-paste.
